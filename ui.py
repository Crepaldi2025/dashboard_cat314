...
def renderizar_sidebar(dados_geo, mapa_nomes_uf):
    with st.sidebar:
        ...
        if opcao == "Skew-T":
            ...
            hoje = datetime.now()
            # Padrão: Hoje (usa a lógica híbrida Forecast/Archive)
...
def renderizar_pagina_principal(opcao):
    ...
    # Exibe instrução apenas se não houver resultados
...
def renderizar_pagina_sobre():
    ...
    try:
        ...
        # Regex corrigido para evitar quebra de linha em strings longas
        pattern = r'<img src="([^"]+)"'
