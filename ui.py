...
def renderizar_validacao_mapa():
    ...
    if st.session_state.tipo_localizacao == "Polígono":
...
def renderizar_pagina_sobre():
    ...
    st.markdown("---")
    st.markdown("""
    Este dashboard foi desenvolvido como parte do projeto da disciplina **CAT314 – Ferramentas de Previsão de Curtíssimo Prazo (nowcasting)**.

    ### Objetivo
    O objetivo principal é fornecer uma interface interativa para a visualização de dados meteorológicos históricos para os municípios do Brasil, utilizando a base de dados **ERA5** através da plataforma **Google Earth Engine**.

    ### Tecnologias Utilizadas
    - **Linguagem:** Python
    - **Dashboard:** Streamlit
    - **Dados Geoespaciais:** Google Earth Engine (GEE)
    - **Visualização de Mapas:** Geemap / Folium / Streamlit-Folium

    ### Fonte dos Dados
    Os dados de temperatura, precipitação e vento são provenientes do [ECMWF ERA5 Land Daily Aggregated](https://developers.google.com/earth-engine/datasets/catalog/ECMWF_ERA5_LAND_DAILY_AGGR#bands), um conjunto de dados de reanálise climática global de alta resolução.
    """)
