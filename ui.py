    ...
    with st.sidebar:
        ...
        if opcao_selecionada in ["Mapas", "Séries Temporais"]:
            ...
            if st.button("Gerar Análise", type="primary", use_container_width=True, disabled=st.session_state.get('date_error', False)):
                ...
                st.session_state.area_validada = True  # garante desbloqueio da execução
...
def renderizar_pagina_sobre():
    ...
    **European Centre for Medium-Range Weather Forecasts (ECMWF)** por meio da plataforma **Google Earth Engine (GEE)**.
    ...
    st.markdown("""
    Os dados utilizados provêm do produto **ERA5-Land Daily Aggregated**, mantido pelo **ECMWF (European Centre for Medium-Range Weather Forecasts)** 
