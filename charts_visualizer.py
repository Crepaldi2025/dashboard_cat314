...
def display_time_series_chart(df: pd.DataFrame, variable: str, unit: str, show_help: bool = True):
    ...
    # 3. GUIA DE ÍCONES E AJUDA (APENAS ESTE FICA CONDICIONAL)
    if show_help:
    ...
    # 4. Estatísticas (AGORA SEMPRE VISÍVEL)
    ...
    # 5. Tabela Profissional e Exportação (AGORA SEMPRE VISÍVEL)
    st.markdown("---")
    # st.subheader("Tabela de Dados") (Comentei o título para economizar espaço no modo múltiplo)
    ...
    try:
        ...
        # CORREÇÃO AQUI: Adicionado key=f"btn_png_{variable_clean}"
        with col_img1: 
            st.download_button(
                "📷 Baixar Gráfico (PNG)", 
                data=img_png, 
                ...
                mime="image/png", 
                use_container_width=True,
                key=f"btn_png_{variable_clean}" # ID ÚNICO
            )
            
        # CORREÇÃO AQUI: Adicionado key=f"btn_jpg_{variable_clean}"
        with col_img2: 
            st.download_button(
                "💾 Baixar Gráfico (JPG)", 
                data=img_jpg, 
                ...
                mime="image/jpeg", 
                use_container_width=True,
                key=f"btn_jpg_{variable_clean}" # ID ÚNICO
            )
    ...
    # 3. Ajuda
    if show_help:
        render_chart_tips() # Assumindo que render_chart_tips existe no main ou foi importado
    ...
    with cex1: 
        st.download_button(
            ...
            mime="text/csv", 
            use_container_width=True,
            key=f"btn_csv_{variable_clean}" # ID ÚNICO
        )
    ...
    with cex2: 
        if excel_data: 
            st.download_button(
                ...
                use_container_width=True,
                key=f"btn_xlsx_{variable_clean}" # ID ÚNICO
            )
