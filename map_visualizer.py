...
# ------------------------------------------------------------------
# 1. MAPA INTERATIVO (Com Fundo de Satélite + Marcador Central)
# ------------------------------------------------------------------
...
def create_interactive_map(ee_image: ee.Image, feature: ee.Feature, vis_params: dict, unit_label: str = ""):
    try:
        ...
        # Tenta obter o CENTRO para o marcador (com margem de erro para evitar falhas)
    ...
    # Cria o mapa centralizado no polígono
    mapa = geemap.Map(center=[lat_c, lon_c], zoom=4)
    
    # --- FUNDO DE SATÉLITE HÍBRIDO (Google) ---
    mapa.add_tile_layer(
        url="https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}",
        name="Google Hybrid",
        attribution="Google"
    ...
    # Adiciona os dados climáticos e o contorno
    mapa.addLayer(ee_image, vis_params, "Dados Climáticos")
    mapa.addLayer(ee.Image().paint(ee.FeatureCollection([feature]), 0, 2), {"palette": "black"}, "Contorno")
    
    # --- MARCADOR COM LAT/LON ---
    folium.Marker(
        location=[lat_c, lon_c],
...
# ------------------------------------------------------------------
# 2. MAPA ESTÁTICO (Leve + Ponto Central Vermelho)
# ------------------------------------------------------------------
...
def create_static_map(ee_image: ee.Image, feature: ee.Feature, vis_params: dict, unit_label: str = "") -> tuple[str, str, str]:
    try:
        ...
        # --- VISUALIZAÇÃO DO PONTO CENTRAL ---
        # Usa maxError=1 para corrigir o erro "Unable to perform... geometry operation"
        geom_center = feature.geometry().centroid(maxError=1)
        center_pt = ee.FeatureCollection([ee.Feature(geom_center)])
        
        # Pinta um ponto vermelho no centro
        center_img = ee.Image().paint(center_pt, 1, 4) # Espessura 4
        center_vis = center_img.visualize(palette=['FF0000']) 
        # -------------------------------------

        # Composição: Dados -> Contorno -> Ponto
        final = visualized_data.blend(outline_vis).blend(center_vis)

        # Definição da Região (Zoom)
        try:
...
# ------------------------------------------------------------------
# 3. FUNÇÕES AUXILIARES (Legenda e Título)
# ------------------------------------------------------------------
